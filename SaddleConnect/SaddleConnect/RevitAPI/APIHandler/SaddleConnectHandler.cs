...
namespace SaddleConnect
{
    ...
    public class SaddleConnectHandler : IExternalEventHandler
    {
                ...
                for (int ii = 0; ii < 100; ii++)
                {
                    ...
                    if (SelectedElements.Count() != 0)
                    {
                        ...
                        using (Transaction tr = new Transaction(doc, "ConPak-VConnect"))
                        {
                            ...
                            if (ParentUserControl.Instance.tagControl.SelectedIndex == 2 || ParentUserControl.Instance.tagControl.SelectedIndex == 3 || ParentUserControl.Instance.tagControl.SelectedIndex == 4)
                                        ...
                                        if (primarySortedElements.Count == secondarySortedElements.Count)
                                        {
                                            ...
                                            else if (isSamDir && Math.Round(groupPrimary.ElementAt(i).Key, 4) == Math.Round(groupSecondary.ElementAt(i).Key, 4))
                                            {
                                                ...
                                                if (isSamDirecheckline)
                                                {
                                                    ...
                                                    SaddleExecute(_uiapp, ref primarySortedElements, ref secondarySortedElements);
        ...
        public void SaddleExecute(UIApplication uiapp, ref List<Element> PrimaryElements, ref List<Element> SecondaryElements)
        {
                ...
                try
                {
                    using (SubTransaction tx = new SubTransaction(doc))
                    {
                        ...
                        for (int i = 0; i < PrimaryElements.Count; i++)
                        {
                            ...
                            /*  Conduit newCon = Utility.CreateConduit(doc, PrimaryElements[i] as Conduit, ConnectorOne.Origin, new XYZ(pickpoint.X + i, pickpoint.Y + i, ConnectorOne.Origin.Z));
                              Conduit newCon2 = Utility.CreateConduit(doc, SecondaryElements[i] as Conduit, ConnectorTwo.Origin, new XYZ(pickpoint.X, pickpoint.Y, ConnectorTwo.Origin.Z));*/
                        ...
                        XYZ axisStart2 = ConnectorTwo.Origin;
                        ...
                        try
                        {
                            ...
                            else if (ParentUserControl.Instance.tagControl.SelectedIndex == 2)
                            {
                                    ...
                                    if (direct.X == 1)
                                    {
                                        if (pickedline.Origin.Y < axisStart.Y)
                                        {
                                    ...
                                    {
                                        if (pickedline.Origin.X < axisStart.X)
                                        {
                                            //left in vertical
                                            XYZ end = new XYZ(axisStart.X, axisStart.Y, axisStart.Z + 10);
                                            Line l1 = Line.CreateBound(axisStart, end);
